...
class VectorQuantizer(torch.nn.Module):
    ...
    # Ensure quantization is performed using consistent dtype
    @autocast(enabled=False)
    def forward(self, z: torch.Tensor) -> Tuple[torch.Tensor, Mapping[Text, torch.Tensor]]:
        # Preserve dtype (BF16 or Float32) instead of forcing float32
        z_dtype = z.dtype
        ...
        # Ensure embedding has the same dtype as z_flattened
        embedding = embedding.to(z_dtype)
        ...
        # Ensure z_quantized has the same dtype as z
        z_quantized = z_quantized.to(z_dtype)
